...
// ERŐSZAKOS CSS A CANVAS JAVÍTÁSÁHOZ
...
// Rendezi az instanciákat a MACHINE_TYPES sorrendje alapján
...
export default function FormFillerEmbed({ formId, onAnswersChange, onFormChange, initialForm, activeAnswer, globals, onRootRef }: Props){
  ...
  const handleA4Ref = useCallback(
    (node: HTMLDivElement | null) => { onRootRef?.(formId, node) },
    [onRootRef, formId]
  )
  ...
  const setInkRect = (baseKey: string, r: {w:number, h:number}) => {
    ...
    if (!old || old.h !== r.h) setVal(rKey, r); // Vízszintes méret ignorálva
  };
              ...
              <FieldRow key={el.id} label={el.label} className="stacked">
                ...
                {inkEnabled && (
                  <div className="ink-box">
                    ...
                    <InlineInkCanvas key={`${formId}:comment:${el.id}`} value={inkValue} onChange={(d)=> setVal(INK_KEY(noteKey), d)} initialRect={getInkRect(noteKey)} emitInitialRect={false} onRectChange={(r) => setInkRect(noteKey, r)} />
                  </div>
                )}
              </FieldRow>
            )
                ...
                <div className="combo" style={{ position:'relative' }}>
                  ...
                  {isOpen && (
                    <div className="combo-menu" role="listbox">
                      ...
                      <div className="combo-actions">
                        <button type="button" className="btn" onClick={() => setVal(`__open__${el.id}`, false)}>Kész</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          if (el.type === 'module_system_survey') {
            ...
            return (
              <div key={el.id} style={{ width:'100%' }}>
                ...
                {((el as SystemSurveyElement).instances || []).map((inst) => {
                  const noteKey  = `sys_note_${inst.id}`;
                  const inkEnabled = !!answers[INK_TOGGLE(noteKey)];
