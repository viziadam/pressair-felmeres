...
//   const item: SurveySummary = {
//     id,
//     companyName: next.globals?.companyName || 'Névtelen cég',
//     updatedAt: next.updatedAt,
//     formCount: Array.isArray(next.forms) ? next.forms.length : 0,
//     // ÚJ ADATOK MENTÉSE:
//     inspectorName: next.globals?.inspectorName,
//     site: next.globals?.site,
//     contactName: next.globals?.contactName
//   }
//   if (i >= 0) idx[i] = item
//   else idx.push(item)
//   writeIndex(idx)
...
//     formCount: 0,
//     // ÚJ ADATOK MENTÉSE:
//     inspectorName: seed.globals.inspectorName,
//     site: seed.globals.site,
//     contactName: seed.globals.contactName
//   })
//   writeIndex(idx)
//   setActiveSurveyId(id)
...
//   // KIVÉVE: Itt volt a `findSurveyIdByCompanyName` ellenőrzés, ami 
//   // megakadályozta, hogy ugyanahhoz a céghez több felmérést ments!
...
//   if (!s) {
//     s = {
//       id, globals: EMPTY, forms: [], answers: {}, files: {}, snapshots: [], updatedAt: nowIso()
//     };
//   }
...
//     writeIndex(idx);
//   } catch (e: any) {
//     console.warn("Helyi mentés nem sikerült, de a szerverre küldés folytatódik!", e);
//   }

//   const result = await postJSON(`${API_BASE}/surveys/${encodeURIComponent(id)}`, s);
//   if (!result) throw new Error("Szerver hiba történt a mentés során.");
...
//       formCount: Array.isArray(srv.forms) ? srv.forms.length : 0,
//       // ÚJ ADATOK MENTÉSE:
//       inspectorName: srv.globals?.inspectorName,
//       site: srv.globals?.site,
//       contactName: srv.globals?.contactName
//     }
//     if (i >= 0) idx[i] = item
//     else idx.push(item)
//     writeIndex(idx)
...
export function saveIndexLocal(idx: SurveySummary[]) { writeIndex(idx); }
export function getActiveSurveyId(): string | null { return localStorage.getItem(KEY_ACTIVE) }
export function setActiveSurveyId(id: string | null) {
  if (id) localStorage.setItem(KEY_ACTIVE, id)
  else localStorage.removeItem(KEY_ACTIVE)
}
...
// ======================================================================
// BIZTONSÁGOS TÖRLÉS / TISZTÍTÁS (Csak azt törli, ami nincs a listában)
// ======================================================================
export function pruneLocalToActive() {
  const active = getActiveSurveyId()
  const idx = readIndex()
  const validIds = new Set(idx.map(x => x.id))
  if (active) validIds.add(active)

  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i) || ''
    if (k.startsWith('surveys/')) {
      const isSpecial = k === KEY_INDEX || k === KEY_ACTIVE
      if (!isSpecial) {
        const extractedId = k.replace('surveys/', '')
        if (!validIds.has(extractedId)) {
          localStorage.removeItem(k)
          i = -1 
        }
      }
...
// ======================================================================
// GOLYÓÁLLÓ INDEX FRISSÍTÉS (Lokális adatok védelmével!)
// ======================================================================
let latestServerIndex: SurveySummary[] = []

export async function refreshIndex(): Promise<SurveySummary[]> {
  const srvIdx = await (async () => {
    try {
      const r = await fetch(`${API_BASE}/surveys/index`, { credentials:'same-origin' })
      if (!r.ok) return [] as SurveySummary[]
      return (await r.json()) as SurveySummary[]
  ...
  const finalMap = new Map<string, SurveySummary>()

  for (const loc of localIdx) {
    finalMap.set(loc.id, loc)
  }

  for (const srv of srvIdx) {
    if (finalMap.has(srv.id)) {
      const loc = finalMap.get(srv.id)!
      loc.updatedAt = srv.updatedAt || loc.updatedAt
      loc.formCount = srv.formCount ?? loc.formCount
      if (srv.companyName && srv.companyName !== 'Névtelen cég') loc.companyName = srv.companyName
      
      // HA A SZERVERRŐL JÖVŐ ADATBAN VAN NÉV, AZT HASZNÁLJUK (pl import), 
      // KÜLÖNBEN MEGTARTJUK A LOKÁLIS NEVET! EZZEL NEM ÍRJA FELÜL "ISMERETLEN"-RE!
      loc.inspectorName = srv.inspectorName || loc.inspectorName
      loc.site = srv.site || loc.site
      loc.contactName = srv.contactName || loc.contactName
    } else {
      finalMap.set(srv.id, srv)
    }
  }

  const mergedArray = Array.from(finalMap.values())
  mergedArray.sort((a,b)=> (b.updatedAt||'').localeCompare(a.updatedAt||''))  
  
  latestServerIndex = mergedArray
  writeIndex(mergedArray)
  try { pruneLocalToActive() } catch {}
...
export function deleteSurvey(id: string): void {
  // 1. Azonnali törlés a memóriából és lokális tárhelyről
  localStorage.removeItem(KEY_SURVEY(id))
  const idx = readIndex().filter(s => s.id !== id)
  writeIndex(idx)
  latestServerIndex = latestServerIndex.filter(s => s.id !== id) 
  
  if (getActiveSurveyId() === id) setActiveSurveyId(null)

  // 2. Törlés a szerverről
  del(`${API_BASE}/surveys/${encodeURIComponent(id)}`).then(()=>{
    refreshIndex().catch(()=>{})
  })
}
...
export function createSurvey(globals: SurveyState['globals']): SurveyState {
  ...
  // Minden metaadatot precízen beírunk a helyi indexbe
  const idx = readIndex()
  idx.push({
    id,
    companyName: seed.globals.companyName || 'Névtelen cég',
...
export async function saveAll(forms: any[], answersMap: Record<string, any>, globals: Globals) {
  ...
  let s = readSurvey(id);
  if (!s) {
    s = { id, globals: EMPTY, forms: [], answers: {}, files: {}, snapshots: [], updatedAt: nowIso() };
  }
  ...
  try {
    ...
    const memIdx = latestServerIndex.findIndex(x => x.id === id);
    if (memIdx >= 0) latestServerIndex[memIdx] = item;
    else latestServerIndex.push(item);

  } catch (e: any) {
    console.warn("Helyi mentés nem sikerült!", e);
  }
...
// ======================================================================
// EGYÉB FUNKCIÓK (Globális adatok szerkesztése is védve lett!)
// ======================================================================
...
export function loadSurvey(id: string): SurveyState | null {
  ...
  getJSON<SurveyState>(`${API_BASE}/surveys/${encodeURIComponent(id)}`).then(s=>{
    if (s) {
      if (cached && cached.globals && cached.globals.companyName) {
         if (!s.globals || !s.globals.companyName) {
           s.globals = cached.globals;
         }
      }
      writeSurvey(s)
    }
...
export function loadAnswers(formId: string): Record<string, any> {
  ...
  getJSON<Record<string, any>>(`${API_BASE}/surveys/${encodeURIComponent(id)}/answers/${encodeURIComponent(formId)}`).then(map=>{
    if (map && s) {
      const next: SurveyState = { ...s, answers: { ...(s.answers||{}), [formId]: map }, updatedAt: nowIso() }
      writeSurvey(next)
    }
  })
  return (s?.answers?.[formId]) ?? {}
...
export async function loadAnswersAsync(formId: string): Promise<Record<string, any>> {
  ...
  try {
    ...
    if (map) {
      ...
      if (s) {
        const next: SurveyState = { ...s, answers: { ...(s.answers || {}), [formId]: (!isEmpty ? map : local) }, updatedAt: nowIso() }
        writeSurvey(next)
      }
      return !isEmpty ? map : local
    }
...
export function saveAllLocal(forms: any[]) {
  const id = getActiveSurveyId();
  if (!id) return;
  updateSurveyLocalOnly(id, (cur) => ({ ...cur, forms, updatedAt: nowIso() }));
}
